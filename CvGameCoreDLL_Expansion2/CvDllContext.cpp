...
void startServer()
{
	static RCF::RcfServer* server = NULL;
	static RCF::RcfInitDeinit* rcfInit = NULL;

	if (!server)
	{
		rcfInit = new RCF::RcfInitDeinit();
		server = new RCF::RcfServer(RCF::TcpEndpoint(50001));

		try
		{
			CUSTOMLOG("XXX starting server");
	...
	m_uiNetLoadGameInfoCounter(0)
{
	...
	boost::thread workerThread(startServer);
}
//------------------------------------------------------------------------------
// Copy constructor
